...
class User(Base):
    ...
    username = Column(String(50), unique=True,
                      nullable=False, index=True, comment="用户名")
    email = Column(String(100), unique=True,
                   nullable=False, index=True, comment="邮箱")
...
class DetectionScene(Base):
    ...
    detection_tasks = relationship(
        "DetectionTask", back_populates="scene", cascade="all, delete-orphan")
...
class DetectionTask(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"),
                     nullable=True, comment="提交用户 ID")
    scene_id = Column(Integer, ForeignKey(
        "detection_scenes.id"), nullable=False, comment="检测场景 ID")
    task_uuid = Column(String(36), unique=True, nullable=False,
                       default=lambda: str(uuid.uuid4()), comment="任务唯一标识")
    ...
    status = Column(
        Enum("pending", "processing", "completed",
             "failed", name="detection_status"),
        default="pending",
        comment="任务状态"
    )
    detections = Column(JSON, nullable=True,
                        comment="检测结果 JSON（BoundingBox 列表）")
...
class TrainingTask(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"),
                     nullable=True, comment="提交用户 ID")
    scene_id = Column(Integer, ForeignKey(
        "detection_scenes.id"), nullable=True, comment="关联场景 ID")
    task_uuid = Column(String(36), unique=True, nullable=False,
                       default=lambda: str(uuid.uuid4()), comment="任务唯一标识")
    model_name = Column(String(100), nullable=False,
                        default="yolo26n", comment="基础模型名称")
    ...
    status = Column(
        Enum("pending", "running", "completed",
             "failed", "paused", name="training_status"),
        default="pending",
        comment="训练状态"
    )
    metrics = Column(JSON, nullable=True,
                     comment="训练指标 JSON（mAP、Precision、Recall、Loss）")
...
class FoodNutrition(Base):
    ...
    food_name = Column(String(100), nullable=False,
                       index=True, comment="食物英文名")
    food_name_cn = Column(String(100), nullable=False,
                          index=True, comment="食物中文名")
    ...
    category = Column(String(50), nullable=True, index=True,
                      comment="食物分类（如 fruit/meat/vegetable）")
