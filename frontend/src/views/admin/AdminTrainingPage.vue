<template>
  <div class="page-shell training-page">
    <section v-if="isPreviewMode" class="preview-notice surface" role="status">
      ...
      <div><b>模型训练管理预览</b><p>以下是本地演示数据；创建、启动和删除操作已禁用。</p></div>
    </section>

    <header class="page-head">
      <div>
        <span class="status-chip"><Cpu :size="16" weight="bold" /> Training API</span>
        <h1 class="page-title">训练任务，统一管理。</h1>
        <p class="page-description">创建模型训练任务，查看全站任务状态与最终指标，并管理已生成的模型文件。</p>
      </div>
    ...
    <section class="task-panel surface" aria-labelledby="training-task-title">
      <header class="panel-heading">
        <div><span>Training queue</span><h2 id="training-task-title" class="section-title">全站训练任务</h2><p>当前后端未提供可恢复的中断能力，本页仅提供启动和状态查看。</p></div>
      ...
      <div v-else-if="!tasks.length" class="feedback-state" role="status">
        <Cpu :size="38" weight="duotone" /><h3>还没有训练任务</h3><p>全站创建的训练任务都会出现在这里。</p><button class="primary-action" type="button" :disabled="isPreviewMode" @click="openCreate">创建训练</button>
      </div>
      <template v-else>
        <div class="desktop-table">
          <table>
            <thead><tr><th>任务与模型</th><th>提交用户</th><th>状态</th><th>执行进度</th><th>配置</th><th>创建时间</th><th><span class="sr-only">操作</span></th></tr></thead>
            <tbody>
              <tr v-for="task in tasks" :key="task.uuid">
                ...
                <td><div class="progress-cell"><div><strong>{{ progressLabel(task) }}</strong><small>{{ progressHint(task) }}</small></div><span><i :style="{ width: progressWidth(task) }" /></span></div></td>
                ...
                <td><time :datetime="task.createdAt || undefined">{{ formatDate(task.createdAt) }}</time></td>
                <td><div class="row-actions"><button type="button" @click="openDetail(task)"><Eye :size="17" />详情</button><button v-if="task.status === 'pending'" type="button" :disabled="isPreviewMode || actionTaskId === task.uuid" @click="startTask(task)"><Play :size="17" />启动</button></div></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="mobile-cards">
          <article v-for="task in tasks" :key="task.uuid" class="task-card">
            ...
            <div class="mobile-progress"><div><strong>{{ progressLabel(task) }}</strong><small>{{ progressHint(task) }}</small></div><span><i :style="{ width: progressWidth(task) }" /></span></div>
            <dl><div><dt>提交用户</dt><dd>{{ taskOwnerLabel(task) }}</dd></div><div><dt>图像尺寸</dt><dd>{{ task.imgSize ? `${task.imgSize}px` : '--' }}</dd></div><div><dt>Batch</dt><dd>{{ formatMetric(task.batchSize) }}</dd></div><div><dt>Epochs</dt><dd>{{ formatMetric(task.epochs) }}</dd></div><div><dt>创建时间</dt><dd>{{ formatDate(task.createdAt) }}</dd></div></dl>
            <footer class="row-actions"><button type="button" @click="openDetail(task)"><Eye :size="17" />详情</button><button v-if="task.status === 'pending'" type="button" :disabled="isPreviewMode || actionTaskId === task.uuid" @click="startTask(task)"><Play :size="17" />启动</button></footer>
          </article>
        </div>
    ...
    <el-dialog v-model="createVisible" class="training-dialog" width="min(620px, calc(100vw - 32px))" destroy-on-close append-to-body>
      ...
      <form id="training-create-form" class="create-form" @submit.prevent="submitCreate">
        ...
        <label><span>训练轮次</span><input v-model.number="createForm.epochs" type="number" inputmode="numeric" min="1" max="1000" required><small>1–1000 轮</small></label>
        <label><span>图像尺寸</span><input v-model.number="createForm.imgSize" type="number" inputmode="numeric" min="320" max="1920" required><small>320–1920 px</small></label>
        <label><span>Batch Size</span><input v-model.number="createForm.batchSize" type="number" inputmode="numeric" min="1" max="256" required><small>1–256</small></label>
    ...
    <el-dialog v-model="detailVisible" class="training-dialog detail-dialog" width="min(1080px, calc(100vw - 32px))" destroy-on-close append-to-body>
      ...
      <div v-else-if="selectedTask" class="detail-content">
        <section class="detail-summary"><div><span class="status-tag" :class="statusMeta(selectedTask.status).tone"><i />{{ statusMeta(selectedTask.status).label }}</span><h3>{{ selectedTask.modelName || '未命名模型' }}</h3><p>{{ selectedTask.uuid }}</p></div><strong>{{ progressLabel(selectedTask) }}</strong></section>
        <dl class="detail-grid"><div><dt>提交用户</dt><dd>{{ taskOwnerLabel(selectedTask) }}</dd></div><div><dt>数据集</dt><dd>{{ selectedTask.dataYaml || '--' }}</dd></div><div><dt>训练轮次</dt><dd>{{ epochLabel(selectedTask) }}</dd></div><div><dt>图像尺寸</dt><dd>{{ selectedTask.imgSize ? `${selectedTask.imgSize}px` : '--' }}</dd></div><div><dt>Batch Size</dt><dd>{{ formatMetric(selectedTask.batchSize) }}</dd></div><div><dt>创建时间</dt><dd>{{ formatDate(selectedTask.createdAt) }}</dd></div><div><dt>开始时间</dt><dd>{{ formatDate(selectedTask.startedAt) }}</dd></div><div><dt>完成时间</dt><dd>{{ formatDate(selectedTask.completedAt) }}</dd></div></dl>
...
const statCards = computed(() => [
  ...
  { key: 'paused', label: '暂停标记', hint: '历史状态记录', value: stats.value.paused, icon: markRaw(Pause), tone: 'paused' },
...
function statusMeta(status) { return ({ pending: { label: '等待中', tone: 'pending' }, running: { label: '运行中', tone: 'active' }, paused: { label: '暂停标记', tone: 'paused' }, completed: { label: '已完成', tone: 'success' }, failed: { label: '失败', tone: 'danger' } })[status] || { label: status || '未知', tone: 'unknown' } }
...
function taskOwnerLabel(task) { return task?.userId !== null && task?.userId !== undefined ? `用户 ${task.userId}` : '系统任务' }
function progressLabel(task) {
  if (task?.status === 'running' && (!Number.isFinite(task.progress) || task.progress <= 0)) return '训练中'
  if (task?.status === 'failed' && (!Number.isFinite(task.progress) || task.progress <= 0)) return '已终止'
  return formatProgress(task?.progress)
}
function progressHint(task) {
  if (task?.status === 'running' && !Number.isFinite(task.currentEpoch)) return '后端未返回实时 Epoch'
  return epochLabel(task)
}
...
async function loadTasks(background = false) {
  ...
  try {
    ...
    const statsAvailable = statResult.status === 'fulfilled'
    stats.value = statsAvailable
      ? normalizeTrainingStats(statResult.value)
      : { total: normalized.total, completed: null, failed: null, running: null, pending: null, paused: null }
    statusDistribution.value = distributionResult.status === 'fulfilled'
      ? normalizeTrainingStatusDistribution(distributionResult.value)
      : statsAvailable ? deriveTrainingStatusDistribution(stats.value) : []
    distributionFallback.value = distributionResult.status === 'rejected' && statsAvailable
    syncPolling()
...
async function submitCreate() {
  ...
  if (!form.modelName || !form.dataYaml) { createError.value = '请填写模型名称和数据集 YAML 路径。'; return }
  if (!Number.isInteger(form.epochs) || form.epochs < 1 || form.epochs > 1000) { createError.value = '训练轮次必须是 1–1000 之间的整数。'; return }
  if (!Number.isInteger(form.imgSize) || form.imgSize < 320 || form.imgSize > 1920) { createError.value = '图像尺寸必须是 320–1920 之间的整数。'; return }
  if (!Number.isInteger(form.batchSize) || form.batchSize < 1 || form.batchSize > 256) { createError.value = 'Batch Size 必须是 1–256 之间的整数。'; return }
